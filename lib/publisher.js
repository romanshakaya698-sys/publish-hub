...
/**
 * 填标题。
 * CSDN 的标题框是 React 受控 input（aria-hidden，初始隐藏），直接用 locator.fill 不会触发
 * React onChange，计数仍是 0。需先用 native value setter + input 事件写入，CSDN 才能识别。
 * 其它平台仍走通用 fill。
 */
async function fillTitle(page, platform, title, log) {
  if (platform.id === 'csdn' || platform.contentMode === 'markdown') {
    const ok = await fillCsdnTitle(page, title, log);
    return ok;
  }
...
/** CSDN 专属标题填充：点击激活仅当隐藏的 input 隐藏时先点占位区，再 native setter 写入 */
async function fillCsdnTitle(page, title, log) {
  const sel = 'input.article-bar__title--input, input.article-bar__title, input[placeholder*="标题"], .article-bar__title input';
  const input = page.locator(sel).first();
  // 若 input 不可见（aria-hidden），先点击占位区激活
  ...
  try {
    ...
    if (val === title || val.length) {
      log(`✓ 已填写标题：${title}（${val.length} 字）`);
      return true;
    }
    log('⚠ 标题写入后回读为空，请手动检查标题框');
    return false;
  } catch (e) {
    log(`⚠ 标题填充失败：${e.message}`);
    return false;
  }
}
...
/**
 * HTML → Markdown（轻量版，覆盖常见文章标签）。
 * CSDN 编辑器源码区只接受 Markdown 源码，必须先转换。
 */
function htmlToMarkdown(html) {
  if (!html) return '';
  let s = html;

  // 代码块：先于 inline 标签处理（防 <code> 被转义）
...
/**
 * CSDN 专属正文填充。
 * CSDN 编辑器**只接受 Markdown 源码**填到源码区（textarea.editor__inner / pre.editor__inner），
 * 不接受富文本 HTML。之前误填 HTML 到右侧 AI 助手聊天 iframe，正是被通用 iframe 分支命中了。
 * 因此流程是「HTML→Markdown→填入 textarea」，避开 contenteditable 与 iframe 通道。
 *
 * 备注：CSDN 点「发布文章」后会弹出版本对话框（封面 / 分类 / 标签 / 原创声明 等），
 * 由 confirm 模式下用户手动确认，机器人不接管。
 */
async function csdnFillContent(page, html, log) {
  ...
  log(`✓ 正文已转 Markdown（${md.length} 字符）`);

  // 1) 精确选择器：源码区 textarea
  const precise = [
  ...
  for (const sel of precise) {
    ...
    try {
      if ((await loc.count()) && (await loc.isVisible().catch(() => false))) {
        await loc.click();
        await loc.fill(md);
        log(`✓ 已填入 CSDN 正文 Markdown（${sel}）`);
        return true;
      }
    } catch { /* next */ }
  }

  // 2) 兜底：主文档里第一个可见 textarea
  // （CSDN 右侧 AI 输入框是 contenteditable 不是 textarea，天然避开）
  const ta = page.locator('textarea').first();
  if ((await ta.count()) && (await ta.isVisible().catch(() => false))) {
    await ta.click();
    await ta.fill(md);
    log('✓ 已填入 CSDN 正文 Markdown（textarea 兜底）');
    return true;
  }

  log('⚠ 未找到 CSDN Markdown 源码区，请手动粘贴正文');
  return false;
}
...
/**
 * CSDN 发布设置页-文章标签填充（在已进入发布设置页后调用）。
 * CSDN 标签面板：点「+ 添加文章标签」(button.tag__btn-tag) 打开弹窗，
 * 弹窗输入框 placeholder「请输入文字搜索，Enter键可添加自定义标签」，
 * 输入标签文字 + Enter 即生成标签 chip；标签弹窗打开时每次可逐个输入。
 * 数据源：文章自带 tags 数组。
 */
async function csdnFillTags(page, tags, log) {
  ...
  try {
    ...
    log(`⚠ 打开标签面板失败：${e.message}`);
    return false;
  }

  // 2) 依次输入每个标签 + Enter
  //    输入框：placeholder 含「Enter键可添加自定义标签」，或用 dialog 里的可见 input
  ...
  for (const t of tags) {
    try {
      ...
      await page.waitForTimeout(250);
      // native setter + input 事件（React 受控）
      await page.evaluate((val) => {
      ...
      log(`⚠ 标签「${t}」添加失败：${e.message}`);
    }
  }

  // 3) 保留标签弹窗打开状态，让标签 chip 可见（不按 Escape，避免连同发布设置页一起关闭）。
  //    若标签弹窗有独立「确定/关闭」按钮可点，否则留给用户手动关闭。
  ...
  // 若没有独立关闭 X，保持弹窗开着（标签 chip 可见），用户可点弹窗外或在设置页继续操作。
  // 避免误用 Escape（Escape 会连发布设置弹窗一起关闭，导致已填标签不可见）。
...
async function publishToPlatform(record, platformId, article, mode, extra) {
  ...
  try {
    ...
    if (mode === 'auto') {
      ...
      // CSDN 专属确认模式：自动点一次「发布文章」进入发布设置页（封面/标签/分类/原创声明），
      // 然后在设置页里自动填文章标签，最后停在设置页等用户确认封面/声明后点「发布」。
      const btn = await findPublishBtn(page, platform).catch(() => null);
      if (btn) {
        try {
          await btn.scrollIntoViewIfNeeded().catch(() => {});
          await btn.click({ force: true });
          log('✓ 已点击「发布文章」，进入发布设置页');
          await page.waitForTimeout(2500);
          // 自动填文章标签（数据源：文章自带 tags；为空则不填，由用户手动）
          const tags = Array.isArray(article.tags) ? article.tags : [];
          if (tags.length) {
            const ok = await csdnFillTags(page, tags, log);
            if (ok) log(`✓ 已自动填入 ${tags.length} 个文章标签：${tags.join('、')}`);
          } else {
            log('ℹ 文章未配置标签，请你在发布设置页里点「+ 添加文章标签」手动添加');
          }
          log('✓ 发布设置页已就绪，请确认封面/声明后点右下角「发布文章」');
        } catch (e) {
          log(`⚠ 自动点击发布失败：${e.message}，请手动点击左下角「发布文章」`);
        }
      } else {
        log('⚠ 未找到「发布文章」按钮，请手动点击左下角「发布文章」');
      }
      entry.status = 'confirm';
      entry.finalUrl = page.url();
    } else {
      // 确认模式（通用）：把发布按钮滚到视口内并高亮，方便用户直接点击
