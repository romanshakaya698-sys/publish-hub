...
const DEFAULTS = [
  ...
  },
  {
    ...
    selectors: {
      // 标题：input.article-bar__title（placeholder「请输入文章标题」）
      title: [
        'input.article-bar__title',
        'input[placeholder*="标题"]',
        '.article-bar__title input',
      ],
      // 正文：CSDN 默认 Markdown 源码区；正文填充由 publisher.csdnFillContent 处理（先切富文本），
      // 此选择器仅作兜底。注意不能写 iframe，否则会误中右侧内置 AI 助手聊天 iframe。
      content: [
